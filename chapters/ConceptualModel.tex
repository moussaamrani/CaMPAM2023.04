...
%\LK{What is the applicability of operators here, since operators capture the changes on the MM? Should this be the applicability of the suggestion/recommendation?}
%\LC{I think the answer is 'Yes'}
%\MA{An actual illustration on the example of what ``applicable'', or even ``conditions'' means would also help.} 
...
%\LK{But this would only work if all recommendations have the same conditions, right?}\LC{I agree}
%\MA{Probably. Without an actual example of \textsf{Condition}s, it is difficult to tweak the MM more than that. Also, the \textsf{Suggestion} MM does not give any internal ``well-formedness'' rules, so what you are talking about may easily fit in that.}
