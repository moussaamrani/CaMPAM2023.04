...
\MA{Is this severity really useful? Where is it subsequently used anyway?}
\LK{I think it could be useful for developers when deciding to perform certain evolution steps, so maybe we can give that as additional benefit.}
...
\HM{Discuss \textit{Make ref opposite and drop ref opposite}} \MA{I have some arguments to say that we could ignore these in a first attempt, because "opposite"s may always be computed, assuming you have a "decent" meta-metamodel API (as EMF/Ecore provides, among others).}
