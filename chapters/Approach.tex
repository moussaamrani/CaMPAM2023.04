...
\begin{itemize}
 ...
 % \LK{But if the class is abstract in the meta-model and still not abstract in the view type, how do I propagate new instances of the class from the view to the model?} \HM{I think it does not make sense VT accessing a feature through an abstract class in the MM. Because view -> model change propagation is not possible then.} 
 ...
 % \LK{Same here, how are unconstrained changes on a view propagated to the models?}\HM{i think these are instance level constraints and not tackaled in this paper?}\LK{Yes and no, for editable view types, part of the definition is also how changes are propagated back to the models. So I think it would make sense to at least think about that view types may need to be adapted in order to reflect the editability of certain parts.} 
