...
%\LC{TODO: think about phrasing of complex changes a bit more.}
%\LK{Does ``semantic knowledge'' make sense in order to connect it with other places in our paper where we talk about ``semantically meaningful''?}\LC{I think so? Why not? But am open to suggestions to change phrasing.}
%\LK{Ah, sorry, I should've made this clearer, I added the ``semantic'' before ``knowledge''.}
% provide more semantically meaningful information.
...
In contrast to the atomic change types, these operators\LC{'atomic change types' == 'primitive operators' and hence 'these operators' refers to 'complex operators? I find the phrasing a bit ambiguous here, due to sudden re-appearance of 'atomic change types' vs. 'primitive operators' sentence before}\LK{No, ``these operators'' should refer to both primitive and complex operators, since I disagree that atomic change type (e.g., create element) and primitive operators (e.g., create class) are the same concept, although they are on a similar level of granularity.}\LC{I'm still not clear on the difference, and in any case think that difference is not clear in the paper as-is either. Do you mean to say that atomic change \textit{types} are generic (e.g. your mentioned create element), and the primitive operators like create class concretize them?} represent semantically meaningful evolution steps on which further analyses and automated co-evolution support can be built.
