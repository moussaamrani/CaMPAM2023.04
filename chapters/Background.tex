...
Finally, in~\cref{sec:MetaModelEvolution}, we explain how \metamodel evolution can be broken down into \LK{} a sequence of individual steps describing the evolution process and how these steps can be detected a posteriori.
...
The main challenge for view-based development is keeping the views on the models of the system consistent.
...
\begin{comment}
For the definition of the VGTs, common model transformation languages, such as QVT \cite{omg_qvt} or ATL \cite{eclipse_atl}, can be used, as well as formal bidirectional model transformation frameworks, such as the \emph{lenses} framework \cite{foster_combinators_2007}.
...
Another approach for the definition of VGTs is \emph{ModelJoin} \cite{burger_model-join_2016}, which is an operator-based model query language.
...
For us to provide \LK{} helpful suggestions or even automate certain co-evolution steps, the description of the evolution steps should contain semantic information about the kind of change that is represented by it.
...
In order to define \LK{} change types for the purpose of analyzing \metamodel evolution, \textcite{herrmannsdoerfer_extensive_2011} define a catalogue of 61 \metamodel evolution operators.
...
%In contrast to the atomic change types, these operators
%\LC{'atomic change types' == 'primitive operators' and hence 'these operators' 
%refers to 'complex operators? I find the phrasing a bit ambiguous here, due to 
%sudden re-appearance of 'atomic change types' vs. 'primitive operators' sentence
%before}\LK{No, ``these operators'' should refer to both primitive and complex 
%operators, since I disagree that atomic change type (e.g., create element) and 
%primitive operators (e.g., create class) are the same concept, although they are
%on a similar level of granularity.}
%\LC{I'm still not clear on the difference, and in any case think that difference
%is not clear in the paper as-is either. Do you mean to say that atomic change 
%\textit{types} are generic (e.g. your mentioned create element), and the 
%primitive operators like create class concretize them?} 
%\MA{I would disagree on ``semantically meaningful'': all operators represent MM 
%manipulations, which may sometimes align with what a modeller would think, but they
%rarely represent the ``meaning'' of \emph{\textbf{why}} (semantics...) a change 
%is done. Anyway, my suggestion is simplifying: ``\emph{These operators represent evolution steps on which [...]}''}
%represent semantically 
%meaningful evolution steps
These operators represent evolution steps on which further analyses and automated co-evolution support can be built.
\LC{Lars, are you ok with preceding simplified sentence suggested by Moussa? Also see the comment thread in source that I commented out, just above this sentence.}
\LK{I don't have any objections regarding the sentence from Moussa, but as I told Hossain, we are using descriptions like ``semantically meaningful'' also in other places in the paper.}\LC{Hossain tells me we currently have 'semantically meaningful' in just two other spots. Shall we remove the mention there as well?}
\LK{I'll check it after my next meeting, but in the end, we need some way of talking about the difference between the basic operators for changes and the ones intended for describing evolution processes.}
\LK{I changed the occurences of ``meaningful'' in this context and left empty comments wherever I did so. Could you check if the text (including the sentences that surround the changed ones) is still coherent and describes the change types as we use them in our paper?}
